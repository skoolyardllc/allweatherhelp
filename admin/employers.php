<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        if(isset($_POST['add']))
        {
            ...
            $sql="insert into users(email, password, type) values('$email', '$password', 2)";
            if($conn->query($sql))
            {
    ...
    if(isset($_GET['token'])&&!empty($_GET['token']))
    {
        ...
        switch ($token) {
            case '1':
                $sql="select up.*, u.id as uid, u.email from users u, user_profile up where u.id=up.u_id and u.type=2";
                $title="All Employers";
                break;
            case  "2":
                $sql="select up.*, u.id as uid, u.email from users u, user_profile up where up.status = 2 and u.type=2 and u.id=up.u_id";
                $title ="Blocked Employers";
                break; 
            case "3": 
                $sql="select up.*, u.id as uid, u.email from users u, user_profile up where up.status = 1 and u.type=2 and u.id=up.u_id";
                $title ="Unblocked Employers";
                break;
            default:
                     ...
                     ?>
                                    <tr> 
                                        ...
                                        <td style="width:30%">
                                            ...
                                            <form method="post">
                                                <a href="viewemployers?token=<?=$detail['uid']?>" class="btn btn-primary"><i class="fa fa-eye">View</i></a>
    ...
    <div class="modal-dialog" >
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Add A Employer</h4>
            </div>
            <form method="post">
